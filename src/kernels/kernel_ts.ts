            ...
            for (let ij = 0; ij < numInteraction[ii]; ij++) {
                ...
                for (let i = particleOffset[0][ii]; i <= particleOffset[1][ii]; i++) {
                    ...
                    for (let j = particleOffset[0][jj]; j <= particleOffset[1][jj]; j++) {
                        const dx = this.particleBuffer[i * 4] - this.particleCount[j * 4];
                        const dy = this.particleBuffer[i * 4 + 1] - this.particleCount[j * 4 + 1];
                        const dz = this.particleBuffer[i * 4 + 2] - this.particleCount[j * 4 + 2];
                        const invDist = 1 / Math.sqrt(dx * dx + dy * dy + dz * dz + eps);
                        const invDistCude = Math.pow(invDist, 3);
                        const s = this.particleBuffer[i * 4 + 3];
                        ax -= dx * s * inv4PI;
                        ay -= dy * s * inv4PI;
                        az -= dz * s * inv4PI;
                    }
